...
for idx_plot, idx in enumerate(np.random.choice(n_obs, 10)):
    ...
    plot(
        data_recons_numint[idx], colors=colors_numint, ax=temp_ax,
        label='Reconstruction NumInt'
    )
    plot(
        data_recons_pace[idx], colors=colors_pace, ax=temp_ax,
        label='Reconstruction PACE'
    )
    plot(
        data_recons_innpro[idx], colors=colors_innpro, ax=temp_ax,
