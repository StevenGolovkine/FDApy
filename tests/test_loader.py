...
class TestReadCsv(unittest.TestCase):
    def setUp(self):
        ...
        self.dense_data = {"x": [1, 2, 3, 4, 5], "y": [2, 4, 6, 8, 10]}
        self.dense_df = pd.DataFrame(self.dense_data)
        self.dense_df.to_csv("dense_test.csv", index=False)

        self.irregular_data = {"x": [1, 2, np.nan, 4], "y": [2, np.nan, 6, 8]}
    ...
    def test_read_csv_irregular(self):
        ...
        np.testing.assert_array_equal(irregular_obj.values[0], np.array([1, 2]))
        np.testing.assert_array_equal(irregular_obj.values[1], np.array([2]))
        np.testing.assert_array_equal(irregular_obj.values[2], np.array([6]))
        np.testing.assert_array_equal(irregular_obj.values[3], np.array([4, 8]))
