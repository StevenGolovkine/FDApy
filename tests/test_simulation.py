...
class TestAddNoiseUnivariateData(unittest.TestCase):
    ...
    def test_output_shape(self):
        ...
        noisy_data = _add_noise_univariate_data(
            self.data, noise_variance=1.0, rnorm=np.random.default_rng(42).normal
        )
...
class TestSimulationUnivariate(unittest.TestCase):
    ...
    def test_add_noise(self):
        ...
        self.assertIsInstance(self.simulation.noisy_data, DenseFunctionalData)

    def test_sparsify(self):
        self.simulation.sparsify(percentage=0.5, epsilon=0.1)
        self.assertIsInstance(self.simulation.sparse_data, IrregularFunctionalData)

    def test_add_noise_and_sparsify(self):
        self.simulation.add_noise_and_sparsify(
            noise_variance=0.1, percentage=0.5, epsilon=0.1
        )
        self.assertIsInstance(self.simulation.noisy_data, DenseFunctionalData)
        self.assertIsInstance(self.simulation.sparse_data, IrregularFunctionalData)
...
class TestSimulationMultivariate(unittest.TestCase):
    ...
    def test_add_noise(self):
        self.simulation.add_noise(noise_variance=0.5)
        self.assertIsInstance(self.simulation.noisy_data, MultivariateFunctionalData)

    def test_sparsify(self):
        self.simulation.sparsify(percentage=0.5, epsilon=0.1)
        self.assertIsInstance(self.simulation.sparse_data, MultivariateFunctionalData)
    ...
    def test_add_noise_and_sparsify(self):
        ...
        self.assertIsInstance(self.simulation.noisy_data, MultivariateFunctionalData)
        self.assertIsInstance(self.simulation.noisy_data.data[0], DenseFunctionalData)
        self.assertIsInstance(self.simulation.sparse_data, MultivariateFunctionalData)
