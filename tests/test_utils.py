...
class TestRowMean(unittest.TestCase):
    ...
    def test_row_mean_with_negative_values(self):
        ...
        expected = np.array([0.33333333, 0.66666667, 1.0])
        result = _row_mean(x)
        self.assertTrue(np.allclose(result, expected))
...
class TestGetDictDimension(unittest.TestCase):
    ...
    def test_get_dict_dimension_unequal_size(self):
        x = {"a": np.array([1, 2, 3]), "b": np.array([[4, 5], [6, 7]])}
        self.assertEqual(_get_dict_dimension(x), (3, 2))

    def test_get_dict_dimension_same_size(self):
        x = {"a": np.array([1, 2, 3]), "b": np.array([[4, 5], [6, 7], [8, 9]])}
...
class TestShift(unittest.TestCase):
    ...
    def test_shift_negative_num(self):
        ...
        num = -2
        fill_value = np.nan
        expected = np.array([3.0, 4.0, 5.0, np.nan, np.nan])
        result = _shift(x, num, fill_value)
        self.assertTrue(np.array_equal(result, expected, equal_nan=True))
...
class TestIntegrationWeights(unittest.TestCase):
    def test_trapz(self):
        x = np.array([1, 2, 3, 4, 5])
        expected_output = np.array([0.5, 1.0, 1.0, 1.0, 0.5])
        np.testing.assert_array_equal(
            _integration_weights(x, method="trapz"), expected_output
        )
    ...
    def test_simpson(self):
        ...
        np.testing.assert_array_almost_equal(
            _integration_weights(x, method="simpson"), expected_output
        )
    ...
    def test_callable_method(self):
        ...
        np.testing.assert_array_equal(
            _integration_weights(x, method=custom_weights), expected_output
        )
    ...
    def test_invalid_method(self):
        ...
        self.assertEqual(
            str(cm.exception),
            "invalid_method not implemented!",
...
class TestComputeCovariance(unittest.TestCase):
    def test_compute_covariance(self):
        ...
        eigenfunctions = np.array([[1, 2, 3], [2, 3, 4], [3, 4, 5]], dtype=np.float64)

        expected_covariance = np.array(
            [[36.0, 50.0, 64.0], [50.0, 70.0, 90.0], [64.0, 90.0, 116.0]],
            dtype=np.float64,
        )
    ...
    def test_input_shape_mismatch(self):
        eigenvalues = np.array([1, 2, 3], dtype=np.float64)
        eigenfunctions = np.array([[1, 2, 3], [2, 3, 4]], dtype=np.float64)
        with self.assertRaises(ValueError):
...
class TestEigh(unittest.TestCase):
    def test_eigh(self):
        ...
        expected_val = np.array([80.0, 20.0])
        expected_vec = np.array([[0.31622777, 0.9486833], [0.9486833, 0.31622777]])
        ...
        np.testing.assert_array_almost_equal(
            np.matmul(np.matmul(eig_vec, np.diag(eig_val)), eig_vec.T), matrix
        )
