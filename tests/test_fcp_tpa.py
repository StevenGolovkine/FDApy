...
class TestEigenDecompositionPenaltyMatrices(unittest.TestCase):
    def test_eigendecomposition_penalty_matrices(self):
        ...
        expected_val_v = np.array([3.00000000e00, 1.00000000e00, 2.56090598e-16])
...
class TestGCV(unittest.TestCase):
    ...
    def test_gcv(self):
        ...
        output = _gcv(
            self.alpha, len(self.rayleigh), self.vector, self.smoother, self.rayleigh
        )
...
class TestFindOptimalAlpha(unittest.TestCase):
    ...
    def test_find_optimal_alpha_dim_3(self):
        ...
        output = _find_optimal_alpha(
            ...
            self.alpha,
            self.eigenvectors,
            (self.eigenvalues_2, self.penalty_matrix),
            "i, j, ijk -> k",
        )
        np.testing.assert_almost_equal(output, expected_output, decimal=4)
...
class TestUpdateVector(unittest.TestCase):
    ...
    def test_update_vector(self):
        v_cross = _compute_denominator(
            self.vectors[1], self.alphas["v"], self.penalty_matrices["v"]
        )
        w_cross = _compute_denominator(
            self.vectors[2], self.alphas["w"], self.penalty_matrices["w"]
        )
        results = _update_vector(
            self.data, self.vectors, 0, 0, v_cross * w_cross, "i, j, kij -> k"
        )
        expected = np.array(
            [
                0.0,
